...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/04 11:55:22 by wschafer          #+#    #+#             */
/*   Updated: 2025/07/04 11:55:29 by wschafer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		if (cache[fd])
			newline = ft_strchr(cache[fd], '\n');
		else
			newline = NULL;
		if (newline)
			return (extract_line(&cache[fd], newline));
		bytes = read_and_append(fd, &cache[fd]);
		str = handle_read_result(bytes, &cache[fd]);
		if (bytes <= 0)
			return (str);
	}
}
